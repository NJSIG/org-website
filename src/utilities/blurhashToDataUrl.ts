...
export const blurHashToDataURL = (hash: string | undefined): string | undefined => {
  ...
  const blurhashSize = Number(process.env.BLURHASH_SIZE) || 32;

  const pixels = decode(hash, blurhashSize, blurhashSize);
  const dataURL = parsePixels(pixels, blurhashSize, blurhashSize);

  return dataURL;
};

// Thanks to https://github.com/wheany/js-png-encoder
const parsePixels = (pixels: Uint8ClampedArray, width: number, height: number): string => {
...
const getPngArray = (pngString: string): Uint8Array => {
...
const generatePng = (width: number, height: number, rgbaString: string): string => {
  ...
  // Functions (for PNG creation)

  const inflateStore = (data: string): string => {
  ...
  const adler32 = (data: string): number => {
  ...
  const updateCrc = (crc: number, buf: string): number => {
    let c = crc;
    let b: number;
  ...
  const crc = (buf: string): number => {
    return updateCrc(0xffffffff, buf) ^ 0xffffffff;
  };

  const dwordAsString = (dword: number): string => {
  ...
  const createChunk = (length: number, type: string, data: string): string => {
    const CRC = crc(type + data);

    return dwordAsString(length) + type + data + dwordAsString(CRC);
  };

  const createIHDR = (width: number, height: number): string => {
    const IHDRdata =
      dwordAsString(width) +
  ...
  for (let y = 0; y < rgbaString.length; y += width * 4) {
    ...
    if (Array.isArray(rgbaString)) {
      ...
      scanline += rgbaString.substring(y, width * 4);
    }
    scanlines += scanline;
  }
